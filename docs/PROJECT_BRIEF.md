...
- DeX·단일창 드롭 파일을 `Download`로 보내는 관리형 Unicode 파일 전송
- 현재 파일/진행률/완료·실패·대기 상태창, 취소와 충돌 파일명 자동 변경
...
기능을 확인했다. 현재 v1.1.0에서는 번들 Scrcpy 4.1, 관리형 Unicode 파일
전송과 실제 ADB 빌드 표시를 추가했으며 공개 전 Windows 7/11 회귀 확인이
필요하다.
...
- Scrcpy용 파일 전송 proxy는 Download 파일 push만 가로채고 나머지 ADB
  명령과 DX Manager 내부 ADB 경로를 바꾸지 않는다.
