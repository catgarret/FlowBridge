...
사라졌을 때만 기본 3분 유예를 시작하며 앱에서 30초·1분·3분·5분·10분 또는 자동
...
실제 Windows를 반복해서 종료하지 않고 같은 경로를 확인할 수 있도록 설정의 진단
페이지에 `Windows 종료 정리 테스트`를 추가했다. 이 기능은 실제 종료와 동일하게
진행 중인 작업 생성기를 막고 모든 연결 기기의 overlay와 절전모드 해제를 병렬로
정리한 뒤 DX Manager만 종료한다. Windows는 종료하지 않으며, 테스트 종료 뒤에는
명시적 앱 종료와 같이 DX Manager 번들 프로세스만 최종 정리한다. .NET Framework
4.6.2 x64 Release 빌드와 총 37개 다중 기기 테스트가 경고·오류·실패 없이 통과했다.
