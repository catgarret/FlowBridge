...
- 현재 작업: v1.3.0 공개 배포 완료, v2 1·2단계 구현 완료

v2 1단계 기준 커밋은 `3534be8`이다. 2단계에서는 `AdbService`의 전역
...
다중기기 기반 독립 테스트 15개가 경고·오류·실패 없이 통과했다.

다음 단계는 기기별 런타임 세션이다. 현재 DeX orchestrator, 단일창 슬롯,
Companion 수신기와 일부 화면 전원 상태는 아직 v1의 단일 활성 기기 구조를
유지한다. 3단계 전에는 다중 기기 UI를 연결하지 않는다.
