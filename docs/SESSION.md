...
- 현재 작업: v1.3.0 공개 배포 완료, v2 다중 기기 1~5단계 구현 완료

5단계 후속 보강으로 설정 창이 메인 기기 탭과 registry 변경을 실시간으로 따라가게
했다. 실제 승인 transport가 무선 하나뿐이면 저장값이 USB여도 무선으로, USB
하나뿐이면 USB로 표시하며 둘 다 연결된 경우에만 저장된 선호값을 유지한다. 탭을
...
Release 빌드와 29개 다중 기기 독립 테스트가 통과했다.
...
4단계에서는 메인 화면 상단에 물리 기기 탭을 추가했다. 탭에는 기기 이름과
...
29개 다중 기기 독립 테스트는 통과했다. 실제 휴대폰 두 대를 연결한 실기 검증에서
...
모든 기기별 무선 프로필을 서로 다른 endpoint로 처리한다. Release 빌드와 29개
독립 테스트가 통과했으며, 다음 실기 확인 대상은 설정에서 두 휴대폰을 번갈아 선택해
각각 무선 준비했을 때 IP와 transport 선택이 섞이지 않는지 여부다.
