...
- 메인 화면 상단에 물리 기기 탭을 추가하고 탭별 독립 DeX·단일창·Companion·양방향
...
- 실제 승인 transport가 하나뿐이면 설정 화면도 현재 USB·무선 상태로 자동
  보정하고, 메인 기기 탭을 바꿔도 설정 창을 유지한 채 대상 정보를 새로 고침
- 휴대폰→PC 수신 파일을 공통 경로 아래 휴대폰 표시 이름별 하위 폴더로 분리
- DeX·단일창 scrcpy 창 제목에 휴대폰 표시 이름을 추가해 동시 실행 창을 구분
- .NET Framework 4.6.2 x64 Release 빌드와 29개 다중 기기 회귀 테스트 통과
