...
DX Manager 관리형 전송은 Scrcpy가 `/sdcard/Download/`로 요청한 일반 파일
push만 처리한다. APK 설치와 그 밖의 ADB 명령은 Scrcpy의 순정 동작을
유지한다. Android 파일명은 UTF-8 기준 255바이트를 넘을 수 없으며, 충돌
접미사를 붙인 최종 이름도 기기 파일시스템 제한을 만족해야 한다.

Scrcpy가 드롭 파일을 한 개씩 요청하므로 상태창은 전체 파일 수를 미리 알지
못하고 현재 파일과 누적 완료·실패·대기 수를 표시한다. 전송 중 Scrcpy 창,
기기 연결 또는 DX Manager가 종료되면 현재 전송을 취소하고 임시 파일 정리를
시도하지만 이미 기기 연결이 사라졌다면 정리가 실패해 로그만 남을 수 있다.
