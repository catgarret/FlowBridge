...
별도로 배포되는 공식 DX Display Cleaner가 설치된 경우에만 진단 페이지에서
권한을 부여할 수 있다. DX Manager는 설치된 `base.apk`의 v2 서명 인증서
SHA-256을 고정된 공식 인증서와 비교하며, 패키지 누락·서명 불일치·ADB 미승인
상태에서는 버튼을 비활성화한다. 다른 서명의 같은 패키지에는 권한을 부여하지
않는다. 권한 부여 직전과 직후에도 다시 검사하고 사후 검증이 실패하면 권한을
즉시 회수한다.

정리 앱은 Android의 단일 전역 `overlay_display_devices` 설정을 삭제하므로,
