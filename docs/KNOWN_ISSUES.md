...
정상 종료에서는 overlay를 제거한다. Windows 세션 종료도 제한 시간 안에서
동일한 overlay 제거와 `절전모드 해제` 복원을 시도한다. 다만 프로세스 강제 종료,
전원 차단, 이미 끊어진 기기 또는 Windows가 정리 시간을 주지 않는 경우에는
Android의 보조 디스플레이나 `절전모드 해제` 상태가 휴대폰에 남을 수 있다.
...
- 설치 프로그램 없음
- 개발 중 Assembly/File version은 `1.3.0.0`
