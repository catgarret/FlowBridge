...
실행 중인 DeX 또는 단일창의 scrcpy 창에 파일을 하나 이상 놓으면 휴대폰의
`Download` 폴더로 복사합니다. DX Manager 파일 전송은 기본값으로 켜져
있습니다. Windows 7/11 호환 경로를 사용해 scrcpy의 순정 Windows 파일 드롭
경로가 보존하지 못할 수 있는 한글과 기타 Unicode 파일명을 보존합니다.

작은 비모달 상태창이 scrcpy 창을 따라가며 현재 파일, 전송 크기 또는 진행률,
완료·실패·대기 수를 표시합니다. **취소**를 누르면 현재 전송을 중단하고
휴대폰의 임시 파일을 정리합니다. 휴대폰에 같은 이름이 있으면 기존 파일을
덮어쓰지 않고 `이름 (1).확장자`, `이름 (2).확장자` 순서로 저장합니다.
...
기존 전송 방식을 유지합니다. APK 설치 드롭과 Download 파일 push 이외의 ADB
명령은 계속 scrcpy의 일반 동작을 사용합니다.
