...
별도로 제공되는 **DX Companion**을 휴대폰에 설치한 경우 진단 페이지의
...
**DX Companion 1.1.0**은 DX Manager가 비정상 종료되었거나 휴대폰 연결이 먼저
끊겼을 때 사용하는 별도 Android 복구 도구입니다. 휴대폰에 남은 보조 디스플레이
시뮬레이션 화면을 제거하고, PC가 복원하지 못한 개발자 옵션의 **절전모드 해제**를
끌 수 있습니다. 평소에는 휴대폰을 연결한 상태에서 **DeX 중지**, `Left Alt+F8`
또는 트레이 메뉴의 **종료**를 사용하는 것이 우선입니다. PC를 다시 연결하지
않고 복구해야 할 때 Companion을 사용하십시오.

### 설치와 최초 1회 권한 부여

1. 별도로 받은 공식 `DX-Companion-v1.1.0.apk`를 휴대폰에 설치합니다.
...
임의 shell 실행 기능은 포함하지 않습니다. 구현된 설정 작업도 보조 디스플레이
시뮬레이션 설정과 Android의 절전모드 해제 개발자 옵션으로 한정됩니다.
