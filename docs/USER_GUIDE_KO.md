...
별도로 제공되는 **DX Display Cleaner**를 휴대폰에 설치한 경우 진단 페이지의
**휴대폰 가상화면 정리 도구**에서 필요한 권한을 한 번 부여할 수 있습니다.
DX Manager는 연결된 휴대폰에서 정확한 패키지명과 공식 APK 서명 인증서를
검증한 경우에만 **정리 앱 권한 부여** 버튼을 활성화합니다. 권한 부여 직전과
직후에도 앱과 권한 상태를 다시 확인하며, 다른 앱에는 권한을 부여하지 않습니다.
...
## 12. 선택형 DX Display Cleaner

**DX Display Cleaner**는 DX Manager가 비정상 종료되었거나 휴대폰 연결이 먼저
끊겨 보조 디스플레이 시뮬레이션 화면이 휴대폰에 남았을 때 사용하는 별도
Android 복구 도구입니다. 평소에는 휴대폰을 연결한 상태에서 **DeX 중지** 또는
DX Manager 정상 종료를 사용하는 것이 우선이며, 정리 앱은 남은 화면을 PC 없이
제거해야 할 때 사용합니다.

### 설치와 최초 1회 권한 부여

1. 별도로 받은 공식 `DX-Display-Cleaner-v1.0.0.apk`를 휴대폰에 설치합니다.
   실행 중인 DeX 또는 단일창 scrcpy 창에 APK 하나를 놓으면 scrcpy의 APK 설치
   동작이 실행됩니다. 휴대폰에서 APK를 직접 열어 설치해도 됩니다.
2. 휴대폰에서 USB 디버깅을 켜고 이 컴퓨터의 RSA 연결을 승인한 뒤 DX Manager와
   연결합니다.
3. **설정 > 진단 > 휴대폰 가상화면 정리 도구**를 엽니다.
4. 상태가 **권한 부여 가능**으로 표시되면 **정리 앱 권한 부여**를 누르고 확인
   창을 승인합니다.
5. **권한 부여됨**으로 바뀌었는지 확인합니다.
...
완전히 삭제했다가 다시 설치하면 Android가 권한도 제거하므로 위 과정을 다시
진행해야 합니다. 이 권한은 일반 Android 권한 창에서 직접 허용할 수 없습니다.

### 앱, 빠른 설정 타일과 홈 화면 위젯

- **앱 본체**: 현재 `overlay_display_devices` 설정과 가상화면 상태를 확인하고
  **가상화면 정리** 또는 **상태 새로고침**을 사용할 수 있습니다.
- **빠른 설정 타일**: 알림창의 빠른 설정 편집에서 **DX 가상화면 정리** 타일을
  추가하면 휴대폰 어디서든 남은 화면을 바로 정리할 수 있습니다.
- **홈 화면 위젯**: 홈 화면 위젯 목록에서 DX 가상화면 위젯을 추가하면 상태를
  확인하고 **정리** 또는 새로고침 버튼을 사용할 수 있습니다. 앱 밖에서 상태가
  바뀌었다면 위젯의 새로고침 버튼을 누르십시오.

상태 표시는 다음과 같습니다.

- 컬러 DX 아이콘: overlay 설정 활성, 정리할 가상화면 있음
- 흑백 DX 아이콘: overlay 설정 없음
...
앱은 `WRITE_SECURE_SETTINGS` 권한 하나만 요청하며 인터넷 권한, 데이터 수집과
임의 shell 실행 기능은 포함하지 않습니다. 구현된 설정 작업도
`overlay_display_devices`의 조회와 삭제로 한정됩니다.
