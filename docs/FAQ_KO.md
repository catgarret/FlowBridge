...
실행 중인 DeX 또는 단일창의 scrcpy 창에 파일을 하나 이상 놓으십시오. 기본
DX Manager 파일 전송은 Windows 7/11 호환 경로로 한글과 기타 Unicode
파일명을 보존해 휴대폰의 `Download` 폴더로 복사합니다.
작은 상태창에서 현재 파일, 진행률, 완료·실패·대기 수와 취소 버튼을 확인할
수 있습니다.

휴대폰의 기존 파일은 덮어쓰지 않습니다. 같은 이름이 있으면 새 파일을
`이름 (1).확장자`, `이름 (2).확장자` 순서로 저장합니다. 전송을 취소하면
휴대폰의 임시 파일도 제거하도록 시도합니다.
