...
별도로 제공되는 공식 **DX Companion**을 설치하고 DX Manager의 **설정 >
진단 > 휴대폰 가상화면 정리 도구**에서 최초 1회 권한을 부여했다면, 앱 본체나
...
이 버튼은 별도로 제공되는 공식 **DX Companion**이 연결된 휴대폰에
설치되어 있고 APK 서명 인증서까지 일치할 때만 활성화됩니다. 휴대폰이 ADB로
승인되지 않았거나, 앱이 설치되지 않았거나, 같은 패키지명의 다른 서명 앱이면
권한을 부여하지 않습니다. 버튼 아래의 상태 설명을 확인하십시오.
