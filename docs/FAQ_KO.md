...
별도로 제공되는 공식 **DX Display Cleaner**를 설치하고 DX Manager의 **설정 >
진단 > 휴대폰 가상화면 정리 도구**에서 최초 1회 권한을 부여했다면, 앱 본체나
빠른 설정 타일·홈 화면 위젯에서도 남은 화면을 제거할 수 있습니다. 자세한
설치와 사용 방법은 [한국어 사용 설명서](USER_GUIDE_KO.md#12-선택형-dx-display-cleaner)를
참조하십시오.
...
DX Manager 1.1.0은 그 다음 `Version ...` 줄을 읽습니다. 번들 scrcpy ADB는
...
이 버튼은 별도로 제공되는 공식 **DX Display Cleaner**가 연결된 휴대폰에
...
앱 본체, 빠른 설정 타일 또는 홈 화면 위젯에서 가상화면을 정리할 수 있습니다.
