...
설정 화면을 열 때 승인된 transport가 무선 하나뿐이면 저장된 선호값과 무관하게
무선 연결로, USB 하나뿐이면 USB 연결로 표시한다. USB와 무선이 모두 승인된
경우에만 사용자가 저장한 선호 방식을 유지한다. 이는 저장된 과거 값보다 현재
연결 사실을 우선 표시하면서도 두 transport 사이의 명시적인 선택은 보존하기
위함이다.
...
## 2026-08 - 기기 탭은 런타임을 선택하되 비선택 세션을 중지하지 않는다

메인 화면 상단의 각 탭은 물리 기기 identity 하나와 그 기기의
`DeviceRuntimeServiceSet` 하나를 가리킨다. 선택 변경은 UI 명령 대상을 바꿀 뿐,
다른 탭에서 실행 중인 DeX·단일창·파일 전송이나 미니바를 중지하지 않는다.
...
유지한다. 다만 이전 transport에 걸린 전송과 ADB reverse는 정리하고 새 serial로
Companion을 다시 구성한다. 프로그램 종료는 현재 탭만이 아니라 생성된 모든
