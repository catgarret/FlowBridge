...
ADB만 작은 proxy로 연결한다. proxy는 `/sdcard/Download/` 단일 파일 push만
DX Manager의 관리 큐로 전달하며 나머지 명령은 실제 ADB에 그대로 넘긴다.
...
휴대폰에서 복원한다. 한 기기의 같은 이름은 덮어쓰지 않고 `(1)`, `(2)`로
충돌을 피한다. 전송은 전역 FIFO로 직렬화하고 scrcpy 세션이 끝나면 해당
전송도 취소한다.
