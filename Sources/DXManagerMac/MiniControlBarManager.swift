...
final class MiniControlBarManager {
    ...
    func sync(sessions: [DXSessionController.SessionInfo], capture: @escaping (CGWindowID, String) -> Void,
              volumeDown: @escaping () -> Void, volumeUp: @escaping () -> Void,
        ...
        for session in sessions {
            ...
            let panel = panels[session.id] ?? makePanel(session: session, windowID: window.id, capture: capture, volumeDown: volumeDown, volumeUp: volumeUp, power: power, stop: stop)
    ...
    private func makePanel(session: DXSessionController.SessionInfo, windowID: CGWindowID, capture: @escaping (CGWindowID, String) -> Void,
        ...
        let panel = NSPanel(contentRect: NSRect(x: 0, y: 0, width: 42, height: 205),
        ...
        panel.contentView = NSHostingView(rootView: VStack(spacing: 8) {
            ...
            Button(action: volumeDown) { Image(systemName: "speaker.minus") }.help("Galaxy 볼륨 낮추기")
            Button(action: volumeUp) { Image(systemName: "speaker.plus") }.help("Galaxy 볼륨 높이기")
