...
check(ADBService.friendlyAppName(for: "com.instagram.android") == "Instagram", "notification app friendly name")
check(ADBService.friendlyAppName(for: "com.example.weather_alert") == "Weather Alert", "notification app fallback name")
...
check(migrated.blockedNotificationPackages.isEmpty, "legacy notification block-list migration")
...
check(Device(serial: "adb-ABC._adb-tls-connect._tcp", state: "device", model: "Galaxy").isWireless, "mDNS wireless transport detection")
check(Device(serial: "192.168.0.2:5555", state: "device", model: "Galaxy").connectionName == "Wi-Fi", "TCP wireless transport label")
check(Device(serial: "ABC123", state: "device", model: "Galaxy").connectionName == "USB", "USB transport label")
...
let secureWindow = "Window #1 Window{abc u0 app/.Secure}\n  mAttrs={}\n    fl=00002000\n  mCurrentFocus=Window{abc u0 app/.Secure}"
check(ADBService.parseProtectedScreen(secureWindow), "focused FLAG_SECURE window parsing")
