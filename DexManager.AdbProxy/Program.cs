    ...
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            try
            {
                ...
                if (PathsReferToSameFile(realAdbPath,
                ...
                if (IsManagedTransferEnabled() &&
                    TryParseManagedPush(args, out push))
                {
            ...
            out PushRequest request)
        {
            ...
            {
                serial = Environment.GetEnvironmentVariable(
                    FileTransferEnvironment.SessionSerial);
            }
        ...
        {
            return string.Equals(value, "-n", StringComparison.Ordinal) ||
                string.Equals(value, "-p", StringComparison.Ordinal) ||
                string.Equals(value, "-q", StringComparison.Ordinal) ||
                string.Equals(value, "-Z", StringComparison.Ordinal) ||
        ...
        private static int RequestManagedTransfer(PushRequest push)
        {
            ...
            try
            {
                    ...
                    PipeOptions.None))
                {
                    ...
                    var response = FileTransferWire.Read<
                        FileTransferResponseMessage>(pipe);
                    if (response == null ||
                        response.Version !=
                            FileTransferEnvironment.ProtocolVersion)
                    {
            ...
            {
                return Fail("DX Manager file transfer connection failed: " +
                    ex.Message);
            }
        }
